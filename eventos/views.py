...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        draw.text((222, 657), f"{participante.username}", font=fonte_nome, fill=(0, 0, 0))
        draw.text((765, 773), f"{evento.nome}", font=fonte_info, fill=(0, 0, 0))
        draw.text((825, 842), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0, 0, 0))
